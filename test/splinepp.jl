...
mutable struct SplinePP
     ...
     function SplinePP(geom)
         ...
         axd       = 4ones(Float64, n1p1)
         axod      = ones(Float64, n1)
         ayd       = 4ones(Float64, n2p1)
         ...
         a1x = -aa1x*( 1. + axm1gamma[2,1]+axm1gamma[n1,1])
         a2x = -aa1x*( 1. + axm1gamma[2,2]+axm1gamma[n1,2])
         a3x =  aa2x*(-1. + 2*axm1gamma[1,1] - axm1gamma[2,1]
                          +   axm1gamma[n1,1] - 2*axm1gamma[n1p1,1])
         a4x =  aa2x*( 1. + 2*axm1gamma[1,2] - axm1gamma[2,2]
                          +   axm1gamma[n1,2] - 2*axm1gamma[n1p1,2])
   
         a1y = -aa1y*( 1. + aym1gamma[2,1]+aym1gamma[n2,1])
         a2y = -aa1y*( 1. + aym1gamma[2,2]+aym1gamma[n2,2])
         a3y =  aa2y*(-1. + 2*aym1gamma[1,1] - aym1gamma[2,1]
                          +   aym1gamma[n2,1] - 2*aym1gamma[n2p1,1])
         a4y =  aa2y*( 1. + 2*aym1gamma[1,2] - aym1gamma[2,2]
                          +   aym1gamma[n2,2] - 2*aym1gamma[n2p1,2])
        new( geom, a1x, a2x, a3x, a4x, a1y, a2y, a3y, a4y,
	    axd, ayd, axod, ayod, axm1gamma, aym1gamma, coef, bcoef)
    end 
end
